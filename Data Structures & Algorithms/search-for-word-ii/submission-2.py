...
class Solution:
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        for i in range(row):
            for j in range(col):
                self.dfs(i, j, row, col, board, root, visit, ret, "")

        
        return list(ret)


    def dfs(self, i, j, row, col, board, node, visit, ret, word):
        ...
        self.dfs(i+1, j, row, col, board, node, visit, ret, word)
        self.dfs(i-1, j, row, col, board, node, visit, ret, word)
        self.dfs(i, j+1, row, col, board, node, visit, ret, word)
        self.dfs(i, j-1, row, col, board, node, visit, ret, word)
        visit.remove((i,j))
